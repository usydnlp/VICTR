...
def drawCaption(convas, captions, ixtoword, vis_size, off1=2, off2=2):
    ...
    for i in range(num):
        ...
        for j in range(len(cap)):
            ...
            #word = ixtoword[cap[j]].encode('utf-8', 'ignore')
            word = ixtoword[cap[j]]
            word_display=str(j)+":"+word[:6]
            #word_display=word_display.decode('utf-8')
            word_display=word_display

            #print(((j + off1) * (vis_size + off2), i * FONT_MAX), '%d:%s' % (j, word[:6]))
            #d.text(((j + off1) * (vis_size + off2), i * FONT_MAX), '%d:%s' % (j, word[:6]),
            d.text(((j + off1) * (vis_size + off2), i * FONT_MAX), word_display, fill=(255, 255, 255, 255))
                   #font=fnt, fill=(255, 255, 255, 255))

            sentence.append(word)
        sentence_list.append(sentence)
    img_txt.save('/content/test.png')
