...
class SG_Attention(nn.Module):
    ...
    *modified based on IBM's initial implementation of :class:`Attention`. 
    ...
    def __init__(self, query_dim, sg_dim=300, attention_type='general'):
        ...
        self.linear_out = nn.Linear(sg_dim, query_dim, bias=False)
    ...
    def forward(self, query_in, context_in):
        ...
        #query = self.linear_trans(query_in) #c[16, 5, 1200] q[16, 12, 256]
        batch_size, output_len = query_in.size(0), query_in.size(1)
        ...
        attention_scores = attention_scores.view(batch_size * output_len, query_len) #16*12, 5
        #[14, 12, 100]

        if self.mask is not None:
            mask = self.mask.repeat(output_len, 1) #16, 5
