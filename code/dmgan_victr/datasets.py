...
class TextDataset(data.Dataset):
    def __init__(self, data_dir, split='train',
                 base_size=64,
                 transform=None, target_transform=None, use_sg=False):
        ...
        if self.use_sg:
            ...
            self.sg_object_inside, self.sg_relationship_inside, _ = self.load_sg_index(split, 'pos_inside')
            del self.name2index_inside
            self.sg_object_outside, self.sg_relationship_outside, _ = self.load_sg_index(split, 'pos_outside')
            del self.name2index_outside
            self.sg_object_left, self.sg_relationship_left, _ = self.load_sg_index(split, 'pos_left')
            del self.name2index_left
            self.sg_object_right, self.sg_relationship_right, _ = self.load_sg_index(split, 'pos_right')
            del self.name2index_right
            self.sg_object_above, self.sg_relationship_above, _ = self.load_sg_index(split, 'pos_above')
            del self.name2index_above
            self.sg_object_below, self.sg_relationship_below, _ = self.load_sg_index(split, 'pos_below')
