...
class condGANTrainer(object):
    ...
    def save_img_results(self, netG, noise, sent_emb, sent_emb_sg, words_embs, words_embs_sg, mask,
                         image_encoder, captions, cap_lens,
                         gen_iterations, name='current'):
        # Save images
        fake_imgs, attention_maps, _, _ = netG(noise, sent_emb_sg, words_embs_sg, mask)
    ...
    def train(self):
        ...
        for epoch in range(start_epoch, self.max_epoch):
            ...
            while step < self.num_batches:
                ...
                fake_imgs, _, mu, logvar = netG(noise, sent_emb_sg, words_embs_sg, mask)
                ...
                if gen_iterations % 1000 == 0:
                    backup_para = copy_G_params(netG)
                    load_params(netG, avg_param_G)
                    #self.save_img_results(netG, fixed_noise, sent_emb, sent_emb_sg,
                    #                      words_embs, words_embs_sg, mask, image_encoder,
        ...
        if cfg.TRAIN.NET_G == '':
            ...
            for _ in range(1):  # (cfg.TEXT.CAPTIONS_PER_IMAGE):
                for step, data in enumerate(self.data_loader, 0):
                    ...
                    ######################################################
                    noise.data.normal_(0, 1)
                    fake_imgs, _, _, _ = netG(noise, sent_emb_sg, words_embs_sg, mask)
    ...
    def analysis(self, dataset, batch_size, save_dir):
        ...
        if cfg.TRAIN.NET_G == '':
            ...
            if self.use_sg:
                ...
                embedding_weight=text_encoder.get_embedding_weights()
                sg_embedding = nn.Embedding.from_pretrained(embedding_weight)
                sg_embedding.weight.requires_grad = False
                sg_embedding.cuda()
                ...
                obj_embedding = nn.Embedding.from_pretrained(torch.from_numpy(self.ind2obj))
                rel_embedding = nn.Embedding.from_pretrained(torch.from_numpy(self.ind2rel)) if self.ind2rel is not None else nn.Embedding.from_pretrained(torch.from_numpy(self.ind2obj))
                att_embedding = nn.Embedding.from_pretrained(torch.from_numpy(self.ind2att)) if self.ind2att is not None else nn.Embedding.from_pretrained(torch.from_numpy(self.ind2obj))
                obj_embedding.cuda()
                rel_embedding.cuda()
                att_embedding.cuda()
                print("obj2vec, rel2vec, att2vec initialized")
            ...
            for step, data in enumerate(self.data_loader, 0):
                ...
                if self.use_sg:
                    ...
                    sg5_obj_embed=obj_embedding(sg5_obj) #[16, 20, d]

                    sg5_att_shape=list(sg5_att.size())
                    sg5_att_embed=att_embedding(sg5_att.view(-1, sg5_att_shape[1]*sg5_att_shape[2]))
                    sg5_att_embed=sg5_att_embed.view(sg5_att_shape[0], sg5_att_shape[1], sg5_att_shape[2], -1) #[14, 20, 10, d] 
 
                    sg5_rel_shape=list(sg5_rel.size())
                    sg5_rel_embed=rel_embedding(sg5_rel.view(-1, sg5_rel_shape[1]*sg5_rel_shape[2]))
                    sg5_rel_embed=sg5_rel_embed.view(sg5_rel_shape[0], sg5_rel_shape[1], sg5_rel_shape[2], -1)
                ...
                for i in range(1):  # 16
                    ...
                    if self.use_sg:
                        #print("##########Apply sg attention")

                        if self.sgattn_mask:
                            #print("##########Apply sg attention mask")
                            sg_mask = sg5_obj_count.view(batch_size, -1)
                            sg_mask_valid = (sg_mask==1)
                            sg_mask_valid = sg_mask_valid.cuda()
                            atten_sg.applyMask(sg_mask_valid)
                        ...
                        sgattn_output, _ = atten_sg(words_embs.transpose(1, 2).contiguous(), sg_embed)
                        #words_embs =  sgattn_output.transpose(1, 2).contiguous()
                        words_embs_sg = torch.cat((words_embs, sgattn_output.transpose(1, 2)), dim=1)
                        sgattn_output = torch.sum(sgattn_output, dim=1, keepdim=False)
                        sent_emb_sg = torch.cat((sent_emb, sgattn_output), dim=1)
